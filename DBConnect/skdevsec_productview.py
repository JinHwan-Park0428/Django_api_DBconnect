...
class SkdevsecProductViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def admin_product_search(self, request):
        ...
        try:
            ...
            if pcode == 0:
                strsql = "SELECT * FROM skdevsec_product WHERE (pname LIKE '%" + psearch + "%' OR pcate LIKE '%" + psearch + "%') ORDER BY pid DESC limit " + str(ppage * 8 - 8) + ", 8"
            elif pcode == 1:
                strsql = "SELECT * FROM skdevsec_product WHERE pname LIKE '%" + psearch + "%' ORDER BY pid DESC limit " + str(ppage * 8 - 8) + ", 8"
            elif pcode == 2:
                strsql = "SELECT * FROM skdevsec_product WHERE pcate LIKE '%" + psearch.upper() + "%' ORDER BY pid DESC limit " + str(ppage * 8 - 8) + ", 8"
            else:
                return Response(0)
